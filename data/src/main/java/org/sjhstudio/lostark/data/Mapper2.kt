...
import org.sjhstudio.lostark.domain.model.JewlInfo.Companion.COOL_DOWN_JEWL
import org.sjhstudio.lostark.domain.model.JewlInfo.Companion.DAMAGE_JEWL
...
internal fun mapperToUserInfo(dto: CharacterInfoDto): CharacterInfo {
    return CharacterInfo(
        ...
        jewl = dto.jewl?.map { jewlInfo ->
            JewlInfo(
                type =
                    if (jewlInfo.jewlName?.contains("멸화") == true) DAMAGE_JEWL
                    else if (jewlInfo.jewlName?.contains("홍염") == true) COOL_DOWN_JEWL
                    else null,
                level = try {
                    jewlInfo.jewlName?.let { jewlName ->
                        var level = "";
